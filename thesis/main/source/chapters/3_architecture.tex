...
A keretrendszer hat elkülöníthetõ részbõl tevõdik össze. Ezek a központi funkciókat összefogó rendszer, a reflexiót és függõség injektálást megvalósító rendszer, az alkalmazás indító rendszer, a grafikus alrendszer, a grafikus megjelenítéshez szükséges ablakozó rendszer, valamint a többi rendszer által, a szkript felület felé szolgáltatott funkciók összessége. A \myref{fig:highlevelov1} és \myref{fig:highlevelov2} ábrákon láthatjuk a rendszer komponenseinek összefoglaló diagramjait. A továbbiakban az egyes összetevõk jellemzõit és azok felépítését részletezzük.

\begin{figure}[ht]
\centering
\epsfig{file=chapters/content/highlevelsystemov1.eps, width=13.65cm, height=11.1cm}
\caption{A keretrendszer komponensei}
\label{fig:highlevelov1}
\end{figure}
...
\begin{figure}[ht]
\centering
\epsfig{file=chapters/content/highlevelsystemov2.eps, width=13.65cm, height=13.505cm}
\caption{A keretrendszer komponensei}
\label{fig:highlevelov2}
\end{figure}
...
A \textit{dependency injection}, azaz "függõség injektálás" egy olyan program architektúra technika, ami egy rendszer, komponensekre történõ bontását és a komponensek közötti függõségek kezelését hivatott segíteni. Virtuális gépen futtatott nyelvek esetében, amikor eredendõen rendelkezésre állnak a \textit{reflexió} technikái, mint a Java-ban, triviálisan megvalósítható és alkalmazható az injektálás módszere. Más a helyzet azonban a C++ környezet esetén, hiszen nem áll rendelkezésre a reflexió és így az injektálás lehetõsége sem.
...
A \textit{register\_class} kifejezés jelenti a belépési pontot a reflektálás rendszerébe. Az ötlet az, hogy egy olyan osztályt definiálunk (\textit{class\_registrator}), ami a konstruktorában regisztrál egy osztályt, a megadott néven. A regisztráció során a névhez hozzárendelõdik egy funkció objektum (\textit{functor}), ami a regisztrátor osztály típus paraméterébõl kerül kialakításra, úgy, hogy a funkció a megadott típust példányosítsa a meghívásakor:
...
Ahhoz, hogy egy osztály részt vehessen a reflexió rendszerében, illetve a függõségek kezelésében, a \textit{component} osztályból kell öröklõdnie (\textit{di\_rtlib/component}). Ez az osztály tárolja a függõségek referenciáit és a reflektált osztály \textit{meta-osztályát}, ezenkívül biztosítja, hogy kizárólag az osztályon belülrõl történhessen függõség deklaráció.

Az osztály függõségeit a \textit{di\_rtlib/define\_dependency.hpp} fájlban található makrókkal lehet deklarálni (illetve lehetséges a makrók használata nélkül is, de jelentõsen bonyolultabbá tenné a megoldást). A technika hasonló a reflektálásnál leírthoz: egy olyan objektumot deklarálunk az osztályban, ami a konstruktorában "automatikusan" regisztrálja a megadott nevet és típust, mint az osztály egy függõségét. A késõbbiekben ezen a néven keresztül lehet majd injektálni az osztály által igényelt komponenst.
...
Az itt látható kódrészletben a tartalom nyilvántartó rendszer egy osztályát látjuk. A \textit{component\_dependency} makró segítségével függõségként deklaráljuk az \textit{i\_logger} interfészt. Az \textit{m\_logger} azonosítóval érhetjük el a komponenst az osztályon belül, mint egy általános változó deklaráció esetében. A komponens rendszer szempontjából pedig, a \textit{logger} nevet kell majd használnunk, ami automatikusan kerül generálásra a változó névbõl (elhagyva az \textit{m\_}, tagváltozók jelölésére használt prefixet).

A makró második változatában, a \textit{component\_dependencies} kifejezés hasonlóképpen deklarál függõséget egy osztály számára. A különbség az, hogy ennek segítségével egy dinamikus tömb változó kerül létrehozásra az osztályon belül, ami tetszõleges számú komponenst tárolhat a megadott típusból (jelen esetben \textit{i\_asset\_manager}).
...
A kódrészlet az üzenetküldõ rendszer "fájl-csatorna" implementációját tartalmazza. A \textit{component\_attribute} makrót használjuk egy karakterlánc (std::string) típusú attribútum deklarálására. Az attribútum azonosítója hasonlóképpen alakul a függõségek deklarálásakor látotthoz: az \textit{m\_file} néven érhetjük el a változót az osztályon belül, a \textit{file} néven pedig a komponens konfigurációból.

\SubSection{Komponens konfiguráció}

A rendszer különbözõ részei végül egy konfigurációs fájlban futnak össze. Jelen esetben ez az \textit{engine.conf} fájl, ami elengedhetetlen a keretrendszer futtatásához. A rendszer a saját könyvtárának gyökerében (vagy önnálló futtatás esetén a futtatható állomány mellett) keresi ezt a konfigurációs állományt.

A konfigurációs fájl egy standard XML formátumú fájl, aminek a struktúráját egy \textit{XSD séma} írja le (\textit{di/di.xsd}). A gyökér elem minden esetben a \textit{DI} kifejezés, ennek közvetlen gyermekei lehetnek a \textit{plugins}, a \textit{components} és a \textit{wiring}.
...
A \textit{wiring} rész alatt szerepelhetnek az \textit{inject}, \textit{inject-one}, \textit{inject-slot} és az \textit{inject-all} tagok, melyek mindegyike a függõségek beállítására alkalmas, különbözõ szempontok szerint. 

Az \textit{inject} tagnak szükséges attribútumként megadni egy fogadó komponenst, egy injektált komponenst, és azt, hogy a fogadó komponens mely függõsége az, amit felold ez az injekció (\textit{slot}).
...
Az \textit{inject-one} tag arra való, hogy egy megadott komponens összes függõségét beállítsuk egy blokkon belül. A \textit{receiver} határozza meg a cél komponenst, a további \textit{inject} tagokkal pedig egy-egy függõséget oldhatunk fel, a válaszott komponensekkel (\textit{injection}).
...
Az \textit{inject-slot} taggal egy megadott elnevezésû függõséget állíthatunk be egyszerre az összes komponensben (amiben szerepel). Például a legtöbb komponens használja az üzenetküldõ rendszert, így célszerû egy taggal injektálni az összes függõséget:
...
Annak érdekében, hogy a különbözõ rendszerfunkciók mûkõdésérõl valósidejû információkat kapjunk, illetve, hogy az esetlegesen felmerülõ hibák rögzítésre kerülhessenek bevezetünk egy üzenetküldõ rendszert.

Az üzenet rendszer implementációja a \textit{core/logger} útvonalon található. A rendszer központi osztálya a \textit{logger} osztály, amin keresztül elérhetõek a fõ funkciók. Az üzenetküldés belépési pontja a \textit{log} függvény, ami egy üzenetprioritást, egy üzenetet és az üzenet küldõ osztály nevét várja paraméterként.
...
A rendszer úgy került megvalósításra, hogy tetszõleges számú \textit{csatorna} adható a rendszerhez, és mindegyik csatorna megkapja az összes küldött üzenetet, amivel aztán az implementációtól függõ mûvelet hajtódik végre.

\Section{A tartalom nyilvántartó alrendszer}

A \textit{core/asset} útvonalon található minden osztály és funkció, ami a nyilvántartó rendszerhez szükséges. Ezek az osztályok alapul szolgálnak a konkrét tartalom típusok implementációihoz, amiket láthatunk majd az alrendszerek részletezésénél.

\SubSection{Tartalom osztályok alapja}

Minden tartalom típus õsosztálya az \textit{asset\_base} osztály, illetve az absztrakt interfész az \textit{i\_asset}. Az alap osztály tartalmaz néhány olyan adattagot, ami bármely tartalom típusra jellemzõ lehet. Ilyen a tartalom fájl útvonalát jelzõ tag (\textit{m\_path:asset\_url}), az objektum aktuális állapota (\textit{m\_state:asset\_state}), valamint egy egyedi konfigurációs objektum, ami egy-egy tartalomhoz kapcsolódó betöltési paraméterek tárolására alkalmas.

Az alaposztály publikus interfésze az említett tagok lekérdezéséhez és/vagy beállításához biztosít funkciókat, illetve a tartalom, memóriába történõ betöltését lehet kezdeményezni rajta keresztül.

\SubSection{Tartalom paraméterek}

Egy tartalom fájl inicializálásakor a rendszer automatikusan ellenõrzi, hogy létezik-e a tartalomról adatokat szolgáltató \textit{meta-fájl}. A fájl elnevezési konvenciója az \textit{eredetinev.meta} formát követi. Ezek alapján a kódrészlet, ami a \textit{meta-fájl} betöltését végzi, a következõképpen alakul:
...
Ha létezik a meta-fájl (ezt a \textit{check\_asset} hívás ellenõrzi), az beolvasásra kerül, majd az adatot az \textit{asset\_params} osztály kapja meg. Az osztály egyetlen feladata, hogy név-érték párokra bontsa le a fájlban megadott paramétereket. A fájl struktúrája, ezen feladat támogatására, a lehetõ legegyszerûbb módon van kialakítva: minden sorban egy névnek és egy értéknek kell szerepelnie, egyenlõség (=) jellel elválasztva, ahogyan az \textit{INI} fájlok esetében szokásos \cite{inifile}.
...
\SubSection{Tartalom típus nyilvántartó osztály}

A tartalom nyilvántartás két lépcsõben történik. Az elsõ lépésben a különbözõ típusokat, és az azokhoz tartozó nyilvántartó objektumokat struktúráljuk. A következõ lépésben pedig, egy adott típushoz tartozó objektumokat rendszerezzük, mint például a textúrák.

Az elsõ lépés megvalósításához kialakított interfész az \textit{i\_asset\_type\_manager}. 
\begin{cpp}
...
\end{cpp}
A feladata egyszerû: valamilyen módon szolgáltatnia kell az összes elérhetõ típus listáját, az összes, típushoz regisztrált nyilvántartó objektumot, valamint egy konkrét típushoz tartozó nyilvántartó objektumot.

Az interfész implementációja egy injektálható komponens, ami a függõségein keresztül éri el az egyes nyilvántartó objektumokat, és az ahhoz tartozó tartalom típusokat (\textit{asset.cpp - asset\_type\_manager}).

\SubSection{Tartalom nyilvántartó osztály}

A nyilvántartó osztály interfésze az \textit{i\_asset\_manager}, melynek funkciói egy-egy tartalom betöltésére illetve felszabadítására vonatkoznak. Az interfész alap implementációja az \textit{asset\_manager\_base}. Ez valósítja meg az interfész minden funkcióját, a \textit{get\_type} híváson kívül, ami arra szolgál, hogy az adott tartalom típus nevét, karakterlánc formátumban adja meg. Ezen kívül az alap megvalósítás bevezet egy új tisztán virtuális függvényt:
\begin{cpp}
