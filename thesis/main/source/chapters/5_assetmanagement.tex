\Chapter{Digitális tartalom nyilvántartás}
\label{chap:assetmanagement}
\label{Chap:tartalom}
A rendszer egy alapvetõ komponense, a tartalom nyilvántartó alrendszer. A következõkben láthatjuk mit nevezünk digitális tartalomnak, hogy milyen felelõsségei vannak egy ilyen alrendszernek és hogy hogyan épül fel a futási környezetben használt, a tartalmat reprezentáló adatstruktúra.

\Section{A digitális tartalom definiálása}
\textit{Digitális tartalom}nak fogunk nevezni minden olyan adatot, ami közvetlenül nem szükséges a rendszer mûködéséhez, tehát a program futtatásához. Nem soroljuk ide a konfigurációs adatokat, hiszen azok általában elengedhetetlenek a program megfelelõ m¿ködéséhez. Ide sorolunk minden olyan adatot, amit a rendszer a futása során valamilyen elõre leprogramozott módon feldolgoz és az így kinyert információt egy jól definiált módon felhasználja.
...
Ebben az esetben az azonosítás megvalósítása egy speciális "útvonal-struktúra", mely implementációja a \textit{core/asset} útvonalon, az \textit{asset\_url} osztályban található. Azért nem hagyományos útvonal formátumot (például Windows operációs rendszer specifikus útvonal formátumot) használunk, hogy a struktúra támogassa a fájl-rendszer és a platform absztrakcióját, ezzel lehetõvé téve például hálózati alapú betöltést, akár különbözõ platformok között is.
...
A hasítótáblák olyan asszociatív adatstruktúrák, melyekben a kulcsok egy hasító algoritmussal kerülnek generálásra, azok pedig közvetlenül indexelik a belsõ tárolót valamilyen, implementációtól függõ ütközés-kezelési stratégiával \cite{hash_table}. Az ütközéskezelési stratégiákra azért van szükség, mert a hasítófüggvény különböz¿ bemenetekhez rendelhet azonos értéket, viszont ennek a valószín¿sége a gyakorlatban nagyon kicsi \cite{has_collision_probs}. Egy elterjedt hasító algoritmus például az \textit{SHA-1}, amelynél az ütközés nagyon ritka, de el¿idézhet¿ \cite{sha1_collision}.
...
Ehhez szükséges az objektumokra aktívan jelen levõ \textit{referenciák} (hivatkozások) számontartása. A C++ környezet biztosít úgynevezett automatikus vagy "okos" mutatókat, jelen esetben az \textit{std::shared\_ptr} struktúra adta lehetõségeket fogjuk kihasználni \cite{shared_ptr}. Ez az osztály - az adatstruktúránkkal ellentétben - másolható, ez teszi lehetõvé, hogy az alrendszeren kívül, vagy az alrendszeren belül különbözõ kódrészek úgy osszák meg az adott objektumot, hogy azt nem másolják le, viszont a referenciák számát mindig számontartsuk. 
...
A párhuzamos programozási technikák és problémák, az információs technológia egy egész alterületét képzik. Az itt tárgyalt struktúrák segítségével csupán a legalapvetõbb versengési problémák kiküszöbölését valósíthatjuk meg.
