...
Ebben a részben találhatunk információkat azt illetõen, hogy hogyan is kezdhetünk el fejleszteni a keretrendszer segítségével és, hogy milyen funkciók biztosítottak a szkript környezeten keresztül.

\Section{Szükséges fájlok}
\SubSection{Alacsony szintû konfiguráció}
A \textit{main/engine.conf} útvonalon található fájl tartalmazza a rendszer különbözõ komponenseinek paramétereit, és azok függõségeit. Itt találhatóak továbbá a nyilvántartó rendszer számára biztosított tartalom-deklarációk is.

Ahhoz, hogy egy tartalom, például egy textúra felhasználható legyen a rendszer számára, szerepelnie kell az adott típus felsorolásában. Tegyük fel például, hogy a \textit{grass.png} nevû textúrát szeretnénk elérhetõvé tenni.

Az elsõ lépésben megkeressük a konfigurációs fájlban, a tartalom típusához tartozó nyilvántartó osztályt. Jelen esetben ez a \textit{texture\_manager} lesz, hiszen egy textúrát kívánunk deklarálni. Minden nyilvántartó osztály rendelkezik az \textit{assets} tömb attribútummal, ebbe kell elhelyeznünk a kívánt értékeket. Tehát a kívánt textúra deklarálása a következõ lesz:

\begin{xml}
<component name="texture_manager" class="texture_manager">
  <attribute name="assets">
    <insert value="texture@grass.png"/>
  </attribute>
</component>
\end{xml}
...
\SubSection{Anyagtulajdonságok deklarációja}
A \textit{main/materials} útvonalon található fájlban láthatjuk az úgy nevezett \textit{material} deklarációkat. Ezek olyan struktúrák, amik meghatározzák, hogy egy adott geometriai objektumot milyen grafikus "árnyékoló" programmal és milyen textúrákkal kell kirajzolni.
...
Képzeljük el ezt úgy, hogy egy gráfnak csak a pontjai adottak, az élei ismeretlenek és különbözõ gráfokat hozunk létre, különbözõ él-halmazok definiálásával. Azért elõnyös ez a megközelítés, mert elõfordulhatnak jelenetenként ismétlõdõ csúcsok, így azonban elegendõ egyszer definiálni õket.
...
Ismerjük már a keretrendszer üzenetküldési lehetõségeit, a \textit{py\_api/logger.py} modul tartalmazza a funkciók hívásához szükséges, egyszerû definíciókat:
...
A funkciók mindegyike egy üzenetet vár, ami a natív üzenetküldõ rendszernek lesz továbbítva a megfelelõ prioritás szerint. Láthatjuk, hogy hiányzik a "fatal" prioritáshoz tartozó hívás, emögött az az elképzelés húzódik, hogy a magas szintû szkriptekben már nem fordulhat elõ olyan hiba, ami az alkalmazás futását közvetlenül akadályozhatná.

\SubSection{Vektorok}

Egyszerû struktúrákban biztosít vektor-algebrai funkciókat a \textit{py\_api/vector.py} útvonalon található modul. Biztosítottak a kettõ-, három- és négydimenziós vektorok, valamint az ezeken értelmezhetõ összeadás és kivonás valamint a számmal történõ szorzás és osztás mûveletei.

A \textit{py\_api/vector\_proxy.py} modul tartalmazza a C-kompatibilitási struktúrákat, amik ahhoz szükségesek, hogy a C-ben és a Python-ban implementált funkciók között megtörténhessen a paraméterek átadása.
